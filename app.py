...
app.py
------
Offline SOC dashboard for SIH26153 — AI based Network Attack Forecasting.

Upload a CICIDS2017 flow CSV (raw, any size — streamed chunk-by-chunk), a
pre-featurized window CSV, or a PCAP (packet-level pipeline runs automatically).
Pick a model (RandomForest / NumPy LSTM) and explore five tabs:
...
Everything runs offline and every number comes from the committed models —
nothing is fabricated and nothing leaves the machine.
"""
...
st.set_page_config(page_title="NetSight — Attack Forecaster", layout="wide")
...
st.title("🛰 NetSight — AI-Based Network Attack Forecaster")
st.caption(
...
with tab1:
    ...
    chart_cols = [c for c in ["window_id", "risk_score", "predicted_alert"]
                  if c in tl.columns]
    line = {"x": tl["window_id"], "y": tl["risk_score"]}
    cdata = tl[chart_cols]
